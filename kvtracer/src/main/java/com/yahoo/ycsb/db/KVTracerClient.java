...
public class KVTracerClient extends DB {
      ...
      if (keymapFileName != null && !keymapFileName.isEmpty()) {
        ...
        if (keymapFile.exists()) {
          ...
          while ((keyline = reader.readLine()) != null) {
            ...
            String prefix = encodeKey("userkey", parts[0], "");
  ...
  public static String unescape(String encStr) {
    ...
    try {
      ...
      mOpWriter.write("r"+" " +  encodeKey(table, key, "field0")+ "\n");
      return Status.OK;
    } catch (IOException exception) {
	...
	try{
        ...
        mOpWriter.write("s"+" " + startkey + " " + Integer.toString(recordcount,10) + "\n");
        return Status.OK;
    } catch (IOException exception) {
      exception.printStackTrace();
      return Status.ERROR;
    ...
    try {
      for (Map.Entry<String, ByteIterator> entry : values.entrySet()) {
        ...
        mOpWriter.write("u"+" " + encKey + " " + value + "\n");
      }
		  return Status.OK;
    } catch (IOException exception) {
      exception.printStackTrace();
    ...
    try {
      ...
      for (Map.Entry<String, ByteIterator> entry : values.entrySet()) {
        String prefix = encodeKey(table, key, "");
        encKey = encodeKey(table, key, entry.getKey());
        HashSet<String> keys = mKeyMap.get(prefix);
        if (keys == null) {
          keys = new HashSet<String>();
          mKeyMap.put(prefix, keys);
      ...
      mOpWriter.write("i"+" " + encKey + " " + insertstr + "\n");
	  return Status.OK;
    } catch (IOException exception) {
      exception.printStackTrace();
      return Status.ERROR;
